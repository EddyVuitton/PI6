...
public class ApplicationService : IApplicationService
{
    ...
    {
        var response = await _httpClient.GetAsync($"api/pi6/GetSolvedForm?fpod_id={fpod_id}");

        if (!response.IsSuccessStatusCode)
        {
            return new formularz_podejscie();
        }

        var responseContent = await response.Content.ReadAsStringAsync();
        var deserialisedResponse = JsonConvert.DeserializeObject<formularz_podejscie>(responseContent);
